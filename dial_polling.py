...
"""\
Dial a number (simple example using polling to check call status)
...
#PORT = sys.argv[1]
BAUDRATE = 115200
#NUMBER = sys.argv[2] # Number to dial - CHANGE THIS TO A REAL NUMBER
PIN = None # SIM card PIN (if any)

from gsmmodem.modem import GsmModem
from gsmmodem.exceptions import InterruptedException, CommandError

def calling(PORT, NUMBER, debug = False):
    ...
    while 1:
        if not wasAnswered:
            ...
            print('Try call again in next 2 hours ...')
            time.sleep(7200)
            caller()
            n += 1
        if n == 10:
