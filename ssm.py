            ...
            for i in self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'service'):
                ...
                if i != 'all':
                    if self.getStatus(self.splitService(i)[0]) != self.splitService(i)[1]:
                        if not self.splitService(i)[2] == 'NOTIFY':
                            self.controlService(self.splitService(i)[0], self.splitService(i)[
                                                1], self.splitService(i)[2], True)
                            if len(self.splitService(i)) == 4:
                                if self.splitService(i)[-1] == 'NOTIFY':
                                    self.notify(self.splitService(
                                        i)[0], self.splitService(i)[1])
                    ...
                    if str(self.splitService(i)[2]).upper() == 'NOTIFY':
                        if not self.splitService(i)[0] in self.servicemonitor_list:
                            if status_monitor.get(self.splitService(i)[0]) != self.getStatus(self.splitService(i)[0]):
                                self.notify(self.splitService(
                                    i)[0], self.getStatus(self.splitService(i)[0]))
                                status_monitor.update(
                                    {self.splitService(i)[0]: self.getStatus(self.splitService(i)[0])})

            time.sleep(int(self.readConfigFile(
                'SIMPLE_SERVICE_MONITOR', 'sleep')[0]))
