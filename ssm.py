#!/usr/bin/python

# Simple Service Monitor
import subprocess
import os
import sys
import argparse
import re
import ConfigParser
from collections import OrderedDict
THIS_PATH = os.path.dirname(__file__)
configfile = 'conf.ini'
import time
import thread
#import sendgrowl
import configset
from gntplib import Publisher,  Resource
import gntplib
import mimetypes
import PySnarl


class MultiOrderedDict(OrderedDict):

    def __setitem__(self, key, value):
        if isinstance(value, list) and key in self:
            self[key].extend(value)
        else:
            super(OrderedDict, self).__setitem__(key, value)
class growl(object):
    def __init__(self, icon = None, stricon = None):
        super(growl, self)
        self.icon = icon
        self.stricon = None

    def publish(self, app, event, title, text, host='127.0.0.1', port=23053, timeout=20, icon=None, iconpath=None):
        if iconpath == None:
            iconpath = self.icon
        if iconpath == None:
            iconpath = self.makeicon(iconpath)
        if icon == None:
            icon = Resource(open(iconpath, 'rb').read())
        # print "app               =", app
        # print "event             =", event
        # print "title             =", title
        # print "text              =", text
        # print "host              =", host
        # print "port              =", port
        # print "timeout           =", timeout
        # print "icon              =", icon
        # print "iconpath          =", iconpath
        # print "-"*220
        publisher = Publisher(app, [event], icon=iconpath, timeout = timeout, host = host, port = port)
        try:
            publisher.register()
        except:
            pass
        try:
            publisher.publish(event, title, text, icon=icon)
        except:
            pass

    def send(self, event, title, text):
        gntplib.publish(event, title, text)

    def makeicon(self, path=None, stricon = None):
        if self.stricon == None:
            self.stricon = stricon
        if os.path.exists(str(path)):
            if "image" in mimetypes.guess_type(path)[0]:
                return os.path.abspath(path)
        else:
            if self.stricon == None: self.stricon = '' 
            imgfile = os.path.join(os.path.dirname(__file__), 'growl.png')
            if isinstance(self.stricon, list):
                print self.stricon
            with open(imgfile, 'wb') as img:
                img.write(self.stricon.decode('base64'))
            return imgfile

class Simple(object):

    def __init__(self):
        super(Simple, self)
        self.servicemonitor_list = []
        self.configfile = configset.get_config_file(configfile)

    def notify(self, svcname='', status=''):
        host = self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl_server')[0]
        port = self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl_port')[0]
        if port != '':
            port = int(port)
        app = self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl_app')[0]
        g = growl()
        if app == '':
            app = 'SSM'
        #icon = g.makeicon(None, stricon=configset.read_config(
            #'SIMPLE_SERVICE_MONITOR', 'growl_icon', self.configfile))
        #icon = g.makeicon(os.path.join(os.path.dirname(__file__), 'growl.png'))
        g.publish(app, 'notify', 'Simple Service Monitor', 'Service %s is %s' % (
            svcname, status), host=host, port=port)
        message = 'Service %s is %s' % (svcname, status)
        PySnarl.snShowMessage(app, message, 3, os.path.join(os.path.dirname(__file__), 'growl.png'))

    def readConfigFile(self, section, option):
        data = []
        cfg = ConfigParser.RawConfigParser(
            allow_no_value=True, dict_type=MultiOrderedDict)
        cfg.read([os.path.join(THIS_PATH, configfile)])
        cfg = cfg.get(section, option)
        for i in cfg:
            if "," in i:
                d1 = str(i).split(",")
                for j in d1:
                    data.append(str(j).strip())
            else:
                data.append(i)
        return data

    def getStatus(self, service):
        d001 = os.popen('sc query "' + str(service) + '"').readlines()
        #print "d001 =", d001
        d002 = re.split(" |\n|:|\b.[0-9]", d001[3])
        #print "d002 =", d002
        try:
            d003 = d002[-3]
            return d003
        except:
            return False

    def getConfig(self, service):
        d001 = os.popen('sc qc "' + str(service) + '"').readlines()
        d002 = re.split(" |\n|:|\t.[0-9]", d001[4])[-2]
        if d002 == 'DEMAND_START':
            return 'DEMAND'
        elif d002 == 'AUTO_START':
            return 'AUTO'
        elif d002 == 'DISABLED':
            return 'DISABLED'

    def splitService(self, services):
        '''
                service = service1:status:config
        '''
        svc = str(services).split(":")
        return svc

    def controlService(self, service, control, config, notify=None):
        '''
                service = [Name of Service]
                control = STOPPED, RUNNING, NOTIFY
                config = DEMAND, DISABLED, AUTO

        '''
        if control == 'STOPPED':
            os.popen("SC STOP \"%s\" > NULL" % (service))
            os.system("SC STOP \"%s\" > NULL" % (service))
            os.popen("net STOP \"%s\" > NULL" % (service))
            os.system("net STOP \"%s\" > NULL" % (service))
            if self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl') == ['1']:            
                self.notify(service, 'STOPPING ...')
        elif control == "RUNNING":
            os.popen("SC START \"%s\" > NULL" % (service))
            os.system("SC START \"%s\" > NULL" % (service))
            os.popen("net START \"%s\" > NULL" % (service))
            os.system("net START \"%s\" > NULL" % (service))
            if self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl') == ['1']:
                self.notify(service, 'STARTING ....')

        os.popen("SC CONFIG %s START= %s" % (service, config))

    def controlService2(self, service, control, config, notify=None):
        '''
                service = [Name of Service]
                control = STOPPED, RUNNING
                config = DEMAND, DISABLED, AUTO

        '''
        if config == 'DEMAND_START':
            config = 'DEMAND'
            self.controlService(service, control, config)
        elif control == "AUTO_START":
            config = 'AUTO'
            self.controlService(service, control, config)
        elif control == 'NOTIFY':
            self.controlService(service, 'NOTIFY', 'NOTIFY')
        os.popen("SC CONFIG %s START= %s" % (service, config))

    def stateAll(self, name=None, state=None):
        list_state = {}
        state_all = {
            1: 'STOPPED',
            2: 'START_PENDING',
            3: 'STOP_PENDING',
            4: 'RUNNING',
            32: 'STOPPED'
        }
        if os.name == 'nt':
            try:
                import win32service as ws
                h = ws.OpenSCManager(None, None, ws.SC_MANAGER_ALL_ACCESS)
                list_service = ws.EnumServicesStatus(h)
                for u in list_service:
                    list_state.update({u[0]: state_all.get(u[2][1])})
            except ImportError:
                slist = os.popen('SC QUERY').readlines()
                for i in slist:
                    dname = re.findall('SERVICE_NAME', i)
                    if dname != []:
                        name = " ".join(re.split(":| |\n|SERVICE_NAME", i))
                        name = name.strip()
                    dstate = re.findall('STATE', i)
                    if dstate != []:
                        state = " ".join(re.split(":| |STATE|[0-9]|\n", i))
                        state = state.strip()
                    list_state.update({name: state})
        return list_state

    def monitor(self):
        '''
                diff service status between real service with config
        '''
        status_monitor = {}
        # print "self.stateAll() =",self.stateAll()
        if 'all' in self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'service') or '*' in self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'service'):
            status_monitor.update(self.stateAll())
        else:
            for c in self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'service'):
                status_monitor.update(
                    {self.splitService(c)[0]: self.getStatus(self.splitService(c)[0])})

        while True:
            if 'all' in self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'service') or '*' in self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'service'):
                # print "AAA"
                if len(status_monitor) != len(self.stateAll()):
                    # print "BBB"
                    for x in self.stateAll():
                        if not x in status_monitor:
                            if self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl') == ['1']:                            
                                self.notify(x, self.stateAll().get(x))
                            status_monitor.update({x: self.stateAll().get(x)})
                else:
                    # print "CCC"
                    # print "status_monitor =",status_monitor
                    for x in status_monitor:
                        # print "x                     =", x
                        # print "status_monitor.get(x) =", status_monitor.get(x)
                        # print "self.getStatus(x)     =", self.getStatus(x)
                        # print "="*220
                        if status_monitor.get(x) != self.getStatus(x):
                            if self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl') == ['1']:
                                self.notify(x, self.getStatus(x))
                            status_monitor.update({x: self.getStatus(x)})
            else:
                for i in self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'service'):
                    # if self.getStatus(self.splitService(i)[0]) !=
                    # self.splitService(i)[1] or
                    # self.getConfig(self.splitService(i)[0]) !=
                    # self.splitService(i)[2]:

                    if self.getStatus(self.splitService(i)[0]) != self.splitService(i)[1]:
                        if self.splitService(i)[2] == 'NOTIFY':
                            #print "002"
                            #if str(self.splitService(i)[2]).upper() == 'NOTIFY':
                                #if not self.splitService(i)[0] in self.servicemonitor_list:
                            if status_monitor.get(self.splitService(i)[0]) != self.getStatus(self.splitService(i)[0]):
                                if self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl') == ['1']:                                
                                    self.notify(self.splitService(i)[0], self.getStatus(self.splitService(i)[0]))
                                status_monitor.update({self.splitService(i)[0]: self.getStatus(self.splitService(i)[0])})                            
                            
                        else:
                            #print "len(self.splitService(i)) =", len(self.splitService(i))
                            #print "self.splitService(i)[1] =", self.splitService(i)[1]
                            if len(self.splitService(i)) == 4:
                                if self.splitService(i)[-1] == 'NOTIFY':
                                    if self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'growl') == ['1']:                                    
                                        self.notify(self.splitService(i)[0], self.splitService(i)[1])
                            if len(self.splitService(i)) == 5 and self.splitService(i)[1] == 'STOPPED':
                                #print "prepare kill..."
                                self.kill_process(self.splitService(i)[0], self.splitService(i)[-1])
                            self.controlService(self.splitService(i)[0], self.splitService(i)[1], self.splitService(i)[2], True)
                           
                time.sleep(int(self.readConfigFile('SIMPLE_SERVICE_MONITOR', 'sleep')[0]))

    def kill_process(self, process, ktype):
        if ktype == 'HARD':
            #print "process kill [hard]"
            a = subprocess.Popen(['px', '-k', str(process + ".exe").strip()])
            step_max = 7
            n = 1
            #print "a.poll =", a.poll()
            while 1:
                if a.poll() == 0:
                    break
                else:
                    a = subprocess.Popen(['px', '-k', str(process+ ".exe").strip()])
                    if a.poll() == None:
                        n += 1
                    if n == step_max:
                        break
                    #print "a.poll 1 =", a.poll()
                    time.sleep(1)
        elif ktype == 'SOFT':
            #print "process kill [soft]"
            a = subprocess.Popen(['px', '-k', str(process).strip()])
            
    def run(self):
        '''
                monitor handle
        '''
        # try:
        # 	thread.start_new_thread(self.monitor, ())
        # except:
        # 	pass
        # finally:
        self.monitor()


if __name__ == '__main__':
    c = Simple()
    c.run()
    #i =  'nlsvc:RUNNING:AUTO:NOTIFY'
    #print c.getStatus(c.splitService(i)[0])
    #print c.splitService(i)[1]
    #print c.splitService(i)[3]
    #print c.readConfigFile('SIMPLE_SERVICE_MONITOR', 'service')
    # c.stateAll()
