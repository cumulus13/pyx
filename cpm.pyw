...
def make_icon():
    ...
    if not os.path.isfile(os.path.join(os.path.dirname(__file__), 'cpm_img.txt')):
        return r'e:\ICONS\clipboard.png'
...
def sent_to_clipserver(clip, host=None, port=11111):
    configname = os.path.join(os.path.dirname(__file__), 'cpm.ini')
    ...
    sock.sendto(bytes(clip.encode('utf-8')), client_address)
    sock.close()
    return True
...
def main(TEXT, dir_only=False, file_only=False, linux_style=False, linux_style2=False, win_style2=False, url_style = False, change_drive_letter = None, no_clipserver=False):
    ...
    try:
        ...
        print ("\n\n")
        sendnotify("ERROR: " + str(traceback.format_exc()), title="ERROR (cpm)", msg="")
