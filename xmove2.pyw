...
class xmove(object):
    ...
    def move(self, src, dst, undo=True, quite=False, auto_rename=True, recursive=None, debug=None, silent=False, hWnd=None):
        ...
        if "*" in src:
            ...
            try:
                ...
                data = 'MOVE: "{0}" --> "{1} ({2}, {3}, {4}, {5}, {6}, {7})"'.format(str(src), str(os.path.abspath(dst)), str(quite), str(auto_rename), str(recursive), str(debug), str(silent), str(hWnd))
                self.logs(data)
                if "?" in src:
                    src, dst1 = src.split("?")
                    if dst1:
    ...
    def run(self, src, dst, undo=True, quite=False, auto_rename=True, recursive=None, move_per_file = False, debug=None, **kwargs):
        #argInfo = inspect.getargvalues(inspect.currentframe()).locals
        if debug:
            self.debugger(locals(), sys._getframe().f_code.co_name)
        src = '\0'.join(src)
        ...
        #if len(src) == 1:
            #self.move(src[0], dst, undo, quite, auto_rename, recursive, debug, **kwargs)
        #elif len(src) > 1:
        if move_per_file:
            self.multi_move(src, dst, undo, quite, auto_rename, recursive, debug, **kwargs)
...
Options:
  ...
  -a, --auto            Whether to generate an automatically-renamed
    ...
    def usage(self):
        ...
        parser.add_argument('-p', '--move-per-file', help = 'Process each of file', action = 'store_true')
                ...
                if options.dest:
                    ...
                    if len(options.PATH) == 1:
                        if "?" in options.PATH[0]:
                            ...
                            dst = os.path.abspath(dst)
                            self.run(src, dst, options.undo, options.quite, options.auto, options.recursive, options.move_per_file, options.debug)
                        else:
                            SRC = os.path.abspath(options.PATH[0])
                            self.run(options.PATH, os.getcwd(), options.undo, options.quite, options.auto, options.recursive, options.move_per_file, options.debug)
                    elif len(options.PATH) > 1:
                        SRC = []
                        for i in options.PATH[0:-1]:
                            SRC.append(os.path.abspath(i))
                        #self.run(options.PATH[0:-1], options.PATH[-1], options.undo, options.quite, options.auto, options.recursive, options.move_per_file, options.debug)
                        self.run(SRC, options.PATH[-1], options.undo, options.quite, options.auto, options.recursive, options.move_per_file, options.debug)
                    else:
