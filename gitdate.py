...
def checkRemote():
	...
	if len(a) < 1:
		...
		if len(q) == 0:
			...
			notify('Add remote origin: %s' % str(q), 'Add Remote', host = ['192.168.1.2:23053'])
			while 1:
			...
			notify('Push to remote origin: %s' % str(q), 'PUSH', host = ['192.168.1.2:23053'])
			while 1:
...
def commit(no_push = False):
	...
	notify("Add file to index", 'Add File', host = ['192.168.1.2:23053'])
	while 1:
		if add.poll() == 0:
			break
		else:
	...
	notify("Commit", 'Commit', host = ['192.168.1.2:23053'])
	while 1:
	...
	notify("Add tag", 'Add Tag', host = ['192.168.1.2:23053'])
	while 1:
		if tag.poll() == 0:
			break
		else:
