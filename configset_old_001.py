...
"""
  ...
  Purpose: Make easy ConfigParser module
  Created: 4/14/2018
"""
...
class configset(object):
    ...
    #def set_config_name(self, filename):
        #global configname
        #configname =filename
        #return configname 
    ...
    def get_config_file(self, filename='', verbosity=None):
        ...
        if os.path.isfile(os.path.join(os.getcwd(), filename)):
            ...
            debug(checking_004 = "os.path.isfile(self.configname)")
            debug(configname = os.path.abspath(self.configname))
            return self.configname
        elif os.path.isfile(os.path.join(os.path.dirname(__file__), self.configname)):
            debug(checking_005 = "os.path.isfile(os.path.join(os.path.dirname(__file__), self.configname))")
            ...
            fcfg = os.path.join(os.getcwd(), self.configname)
            #fcfg = os.path.join(os.path.dirname(__file__), self.configname)
            f = open(fcfg, 'w')
            f.close()
            filecfg = fcfg
    ...
    def read_config2(self, section, option, filename='', verbosity=None): #format ['aaa','bbb','ccc','ddd']
        ...
        cfg = ConfigParser.RawConfigParser(allow_no_value=True, dict_type=MultiOrderedDict) 
        self.cfg.read(filename)
        cfg = self.cfg.get(section, option)
        return cfg
    ...
    def read_config4(self, section, option, filename='', verbosity=None): #format result: [aaa.bbb.ccc.ddd, eee.fff.ggg.hhh, qqq.xxx.yyy.zzz]
        """
            option: section, option, filename=''
            format result: [aaa.bbb.ccc.ddd, eee.fff.ggg.hhh, qqq.xxx.yyy.zzz]
    ...
    def read_all_config(self, filename='', section=[], verbosity=None):
        ...
        dbank = []
        if len(section) != 0:
            for x in self.cfg.options(section):
                d = self.cfg.get(section, x)
                data.update({x:d})
            dbank.append([section,data])        
        else:    
            #print "self.cfg.sections() =", self.cfg.sections()
            for i in self.cfg.sections():
                section.append(i)
                for x in self.cfg.options(i):
                    d = self.cfg.get(i, x)
                    data.update({x:d})
                dbank.append([i,data])
        #print "dbank =",  dbank
    ...
    def test(verbosity=None):
        ...
        self.cfg.read(filename)
        data = self.cfg.sections()
        print self.cfg.get('router','host')
        print data
    def usage(self):
