...
__help__ = """  This is a multiple choise copier 
    ...
    def __moved(self,x,_to=sys.argv[-1],excp=None):
        ...
        if excp == None:
            if os.path.isfile(x) == True:
                try:
                    shutil.copy(x, _to)
                    print " COPYED FILE: \"" + str(x) + "\" --> \"" + os.path.abspath(sys.argv[-1]) + "\""
                    self.logs("COPYED FILE: \"" + str(x) + "\" --> \"" + os.path.abspath(sys.argv[-1]) + "\"")
                except:
                    #return __error__
                    print " COPYED FILE [ERROR]: \"" + str(x) + "\" --> " + os.path.abspath(_to) + " [ERROR]"
                    self.logs("COPYED FILE [ERROR]: \"" + str(x) + "\" --> " + os.path.abspath(_to) + " [ERROR]")
                ...
                try:
                    #print "BBB"
                    shutil.copy2(x, _to)
                    print " COPYED DIRECTORY: \"" + str(x) + "\" --> " + os.path.abspath(sys.argv[-1])
                    self.logs("COPYED DIRECTORY: \"" + str(x) + "\" --> " + os.path.abspath(sys.argv[-1]))
                except:
                    #return __error__
                    print " COPYED DIRECTORY [ERROR]: \"" + str(x) + "\" --> \"" + os.path.abspath(_to) + "\" [ERROR]"
                    self.logs("COPYED DIRECTORY [ERROR]: \"" + str(x) + "\" --> \"" + os.path.abspath(_to) + "\" [ERROR]")
                    self.result_error.append(os.path.abspath(str(x)))
            else:
                print "\n"
                print __help__
            ...
            if excp == x:
                ...
                if os.path.isfile(x) == True:
                    try:
                        if not shutil.Error:
                            shutil.copy(x, _to)
                            print " COPYED FILE: \"" + str(x) + "\" --> \"" + os.path.abspath(_to) + "\""
                            self.logs("COPYED FILE: \"" + str(x) + "\" --> \"" + os.path.abspath(_to) + "\"")
                        else:
                    ...
                    try:
                        if not shutil.Error:
                            shutil.copy2(x, _to)
                            print " COPYED DIRECTORY: \"" + str(x) + "\" --> \"" + os.path.abspath(_to) + "\""
                            self.logs("COPYED DIRECTORY: \"" + str(x) + "\" --> \"" + os.path.abspath(_to) + "\"")
                        else:
                            if x != os.path.abspath(excp):
                                self.result_overwrite.append(x)
                                self.publicTo = _to
        ...
        for i in  range (1,int(len_arg) - 1):
            if os.path.isfile(sys.argv[i]) == True:
                try:
                    ...
                    self.logs("COPYED FILE: \"" + sys.argv[i] + "\" --> " + os.path.abspath(sys.argv[-1]))
                except:
                    #return __error__
                    print " COPYED FILE [ERROR]: \"" + sys.argv[i] + "\" --> " + os.path.abspath(sys.argv[-1] + " [ERROR]")
                    self.logs("COPYED FILE [ERROR]: \"" + sys.argv[i] + "\" --> " + os.path.abspath(sys.argv[-1] + " [ERROR]"))
                ...
                try:
					...
					if os.path.isdir(os.path.join(to_, os.path.basename(sys.argv[i]))):
						print " COPY DIRECTORY: \"" + sys.argv[i] + "\" --> " + os.path.abspath(sys.argv[-1])
					else:
						pass
                except:
