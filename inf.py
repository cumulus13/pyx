        ...
        for i in props:
            if isinstance(props.get(i), dict):
                for j in props.get(i):
                    if j == 'ProductVersion':
                        if props.get(i).get(j) != None:
                            version = props.get(i).get(j).strip()
                    if j == 'ProductName':
                        if props.get(i).get(j) != None:
                            name = props.get(i).get(j).strip()
                    if name == None and j == 'FileDescription':
                        if props.get(i).get(j) != None:
                            name = props.get(i).get(j).strip()
                    if version == None and j == 'FileVersion':
                        if props.get(i).get(j) != None:
                            version = props.get(i).get(j).strip()
                ...
                if name == None and i == 'FileDescription':
                    if props.get(i) != None:
                        name = props.get(i).strip()
                if version == None and i == 'FileVersion':
                    if props.get(i) != None:
                        version = props.get(i).strip()
    if verbosity:
        print "NAME    =", name
    ...
    if len(sys.argv) == 1:
        ...
        if args.rename:
            ...
            for i in props:
                if isinstance(props.get(i), dict):
                    for j in props.get(i):
                        print j, " " * (20 - len(j)), " =", unicode(props.get(i).get(j)).encode('utf-8')
                        # print j
                else:
