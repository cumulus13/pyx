...
class process:
    ...
    def moveAll(self,listFrom=None,listTo=None):
        if listFrom != None:
            for i in listFrom:
                os.system("move /Y \"" + str(i) + "\"" + " \"" + listTo + "\" > nul")
        else:
            ...
            for i in self.result_overwrite:
                if i != self.result_overwrite[-1]:
                    ...
                    if q == "y" or q == "Y" or q == "Yes" or q == "yes" :
                        try:
                            os.system("move /Y \"" + _from + "\"" + " \"" + _to + "\" > nul")
                        except:
    ...
    def __moved(self,x,_to=sys.argv[-1],excp=None):
        ...
        if excp == None:
            if os.path.isfile(x) == True:
                try:
                    # shutil.move(x, _to)
                    # os.system("move \"%s\" \"%s\" /y" %(x, _to))
                    os.system("move /Y \"" + x + "\"" + " \"" + _to + "\" > nul")
        ...
        for i in  range (1,int(len_arg) - 1):
            if os.path.isfile(sys.argv[i]) == True:
                try:
                    ...
                    # os.system("move \"%s\" \"%s\" /y" %(sys.argv[i], to_))
                    os.system("move /Y \"" + sys.argv[i] + "\"" + " \"" + to_ + "\" > nul")
